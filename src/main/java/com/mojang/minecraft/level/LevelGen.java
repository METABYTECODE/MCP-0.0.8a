...
public class LevelGen {
   ...
   public byte[] generateMap() {
      ...
      for(y = 0; y < x; ++y) {
         float fx = this.random.nextFloat() * (float)w;
         float fy = this.random.nextFloat() * (float)d;
         ...
         for(int l = 0; l < length; ++l) {
            fx = (float)((double)fx + Math.sin((double)dir1) * Math.cos((double)dir2));
            z = (float)((double)z + Math.cos((double)dir1) * Math.cos((double)dir2));
            fy = (float)((double)fy + Math.sin((double)dir2));
            dir1 += dira1 * 0.2F;
            dira1 *= 0.9F;
            ...
            for(int xx = (int)(fx - size); xx <= (int)(fx + size); ++xx) {
               for(int yy = (int)(fy - size); yy <= (int)(fy + size); ++yy) {
                  for(int zz = (int)(z - size); zz <= (int)(z + size); ++zz) {
                     float xd = (float)xx - fx;
                     float yd = (float)yy - fy;
                     float zd = (float)zz - z;
